...
class BurpExtender(IBurpExtender, IScannerCheck, ITab):
    ...
    def registerExtenderCallbacks(self, callbacks):
        ...
        callbacks.setExtensionName("Potential XSS")
        ...
        print("Search Input returned in response (reflected)")
    ...
    def doPassiveScan(self, baseRequestResponse):
        ...
        for grep_string in self.grep_strings_list:
            ...
            if (len(matches) > 0):
                print("Match Input returned in response - Potential XSS Reflected:", grep_string.encode('utf-8'))
                issue = CustomScanIssue(
                    baseRequestResponse.getHttpService(),
                    self._helpers.analyzeRequest(baseRequestResponse).getUrl(),
                    [self._callbacks.applyMarkers(baseRequestResponse, None, matches)],
                    "Input returned in response, check!",
    ...
    def _initUI(self):
        ...
        info_label = JLabel("Search for returned input in Html response:")
        gbc.gridx = 0
        gbc.gridy = 0
        gbc.gridwidth = 3
